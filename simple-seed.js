...
const seedCars = async () => {
  try {
    ...
    // Insert cars one by one to catch any errors
    ...
    for (const carData of cars) {
      try {
        ...
        console.error(`❌ Failed to add: ${carData.make} ${carData.model}`, error.message);
      }
    }

    console.log(`\n🎉 Successfully inserted ${insertedCars.length} out of ${cars.length} cars`);

    // Verify final count
    const finalCount = await Car.countDocuments();
    console.log(`📊 Total cars in database after seeding: ${finalCount}`);
