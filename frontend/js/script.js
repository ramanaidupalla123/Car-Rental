...
// Show notification - SIMPLE WHITE VERSION
function showNotification(message, type = 'info') {
    ...
    notification.innerHTML = `
        <div style="display: flex; align-items: flex-start; justify-content: space-between; gap: 0.75rem;">
            ...
            <button style="background: none; border: none; color: #6b7280; cursor: pointer; flex-shrink: 0; padding: 0.25rem; border-radius: 4px; transition: all 0.2s ease;" 
...
function initializeMobileNavigation() {
    ...
    document.querySelectorAll('.mobile-nav-link').forEach(link => {
        link.addEventListener('click', function(e) {
            if (this.getAttribute('href') === 'admin.html') {
                return; // Don't close for admin link
            }
            e.preventDefault();
            ...
            setTimeout(() => {
                if (targetSection && targetSection !== 'admin') {
    ...
    // Initialize scroll to top when opening
...
// Update mobile menu content based on user state
function updateMobileMenuContent() {
    console.log('🔄 Updating mobile menu content...');
    
    const mobileUserInfo = document.getElementById('mobileUserInfo');
    const mobileAuthButtons = document.getElementById('mobileAuthButtons');
    const mobileUserDisplay = document.querySelector('.mobile-user-display');
    const adminLinkMobile = document.querySelector('.admin-nav-link-mobile');
    ...
    if (currentUser) {
        ...
        mobileUserDisplay.style.display = 'flex';
        
        // Update user details
        document.querySelector('.mobile-user-details .mobile-user-name').textContent = currentUser.name;
        document.querySelector('.mobile-user-details .mobile-user-email').textContent = currentUser.email;
        document.querySelector('.mobile-user-display .mobile-user-name').textContent = currentUser.name.split(' ')[0];
        
        // Show admin link if user is admin
        if (currentUser.role === 'admin') {
            adminLinkMobile.style.display = 'flex';
            if (bookingsLink) bookingsLink.style.display = 'none';
        } else {
            adminLinkMobile.style.display = 'none';
            if (bookingsLink) bookingsLink.style.display = 'flex';
        }
    } else {
...
// Quick booking function for mobile
function showQuickBooking() {
    if (!currentUser) {
        showNotification('Please login for quick booking', 'warning');
        showLogin();
        closeMobileMenu();
        return;
    }
    
    // Find a popular car for quick booking
    const popularCar = cars.find(car => car.make === 'Mahindra' && car.model === 'Thar') || cars[0];
    if (popularCar) {
        closeMobileMenu();
        setTimeout(() => {
            showBookingForm(popularCar._id);
        }, 300);
    }
}

// Enhanced scroll to cars function
function scrollToCars() {
    closeMobileMenu();
    setTimeout(() => {
...
// Enhanced updateUI function to handle mobile
function updateUI() {
    console.log('🔄 Updating UI, currentUser:', currentUser);
